...
from models.modified_linear import SplitCosineLinear, CosineLinear
...
class get_model(nn.Module):
    def __init__(self, args, num_classes=40, normal_channel=True):
        ...
        self.conv5 = nn.Sequential(nn.Conv1d(512, args.emb_dims, kernel_size=1, bias=False),
                                   self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.fc1 = nn.Linear(args.emb_dims * 2, 512, bias=False)
    ...
    def forward(self, x, feat=False, rd=False):
        ...
        x = F.leaky_relu(self.bn6(self.fc1(x)), negative_slope=0.2)  # (batch_size, emb_dims*2) -> (batch_size, 512)
        x = self.dp1(x)
        x = F.leaky_relu(self.bn7(self.fc2(x)), negative_slope=0.2)  # (batch_size, 512) -> (batch_size, 256)
        x = self.dp2(x)
        if rd:
